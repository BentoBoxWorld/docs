...
### What can I do when my world size becomes too big?

If your BentoBox world folder has grown huge and you want to shrink it, run a **purge**. Since BentoBox 3.15.0 this is built in — BentoBox deletes the underlying region (`.mca`) files directly, so you no longer need a third-party tool such as Regionerator.

The two big space-eaters are (1) old island regions left behind after resets, and (2) chunks generated by players who never came back. There are two ways to reclaim that space:

**Automatic housekeeping (on by default).** When an island is reset it is *soft-deleted* and a scheduled sweep reaps its region files in the background. This is controlled in BentoBox's `config.yml` under `island.deletion.housekeeping`:

```yaml
island:
  deletion:
    housekeeping:
      deleted-sweep:        # reaps regions for islands already flagged deleted (e.g. resets)
        enabled: true       # on by default
        interval-hours: 24
      age-sweep:            # reaps regions untouched for a long time, reset or not
        enabled: false      # off by default — enable for the most aggressive size control
        interval-days: 30
        min-age-days: 60
```

**Manual purging** — run on demand from the console or in-game:

- `/[admin_command] purge deleted` — immediately reap region files for every island already flagged as deleted (resets, `/[admin_command] delete`).
- `/[admin_command] purge <days>` — reap regions for islands whose owners haven't logged in for `<days>` days and whose region files are at least that old.
- `/[admin_command] purge unowned` — flag every unowned island as deletable so the next sweep removes it.

Notes:

- **Restart the server after a large purge.** The files are removed from disk immediately, but Paper caches recently-loaded chunks in memory; a restart clears that cache and fully releases the space.
- Purge-protected islands, spawn islands, and (with the Level addon) islands above the configured purge level are always skipped.
