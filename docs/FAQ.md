...
The two big space-eaters are (1) chunks generated by players who never came back, and (2) old island regions left behind after resets when `keep-previous-island-on-reset: true`. To recover space:

- Set `keep-previous-island-on-reset: false` in the BentoBox config so reset islands are deleted instead of abandoned.
- Use `/[admin_command] purge` to remove islands of inactive players, then run `/[admin_command] purge regions` to delete the actual world region files. **Always back up the world folder first.**
